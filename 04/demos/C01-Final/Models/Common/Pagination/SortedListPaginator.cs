namespace Models.Common.Pagination
{
  internal class SortedListPaginator<T>: IPaginated<T>
  {
    ...
    public IPage<T> this[int offset] => new ProjectingPage<T>(this.Items, this.Ordinal(offset), this.PageSize);

    private int Ordinal(int offset) => offset.InRange(0, PagesCount, nameof(offset)) + 1;
